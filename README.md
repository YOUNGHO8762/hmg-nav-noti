...
현대자동차그룹 내비게이션 공식 업데이트 웹사이트에서 특정 브랜드의 특정 차량번호와 소유주 정보로 최신 업데이트 배포월을 확인하고, 새로운 업데이트가 감지되면 슬랙(Slack)으로 알림을 보내주는 자동화 스크립트입니다.
...
명령어 뒤에 `"브랜드"`, `"차량번호"`, `"소유주"`를 인자로 넘겨서 실행할 수 있습니다.

```bash
pnpm start "현대" "12가3456" "홍길동"
```

또는 직접 node 명령어로 실행할 수도 있습니다.

```bash
node scraper.js "현대" "12가3456" "홍길동"
```
...
`.github/workflows/scraper.yml` 파일을 열고, `env` 환경변수 부분의 브랜드, 차량번호, 소유주를 본인의 정보에 맞게 수정합니다.

```yaml
env:
  CAR_BRAND: '현대' # <--- [브랜드 입력] 예: 현대, 기아, 제네시스
  CAR_NO: '12가3456' # <--- [차량번호 입력] 예: 12가3456
  CAR_OWNER: '홍길동' # <--- [소유주 입력] 예: 홍길동
```
...
- **자동 실행**: 매일 한국시간 09:00(UTC 00:00)에 스케줄러에 의해 자동 실행됩니다.
